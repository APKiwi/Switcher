...
**HotkeyManager.swift**
- **Option triggers (fork-only)**: Option+Tab and Option+Shift+Tab are registered globally alongside the Command pair, and Option+backtick mirrors the native Cmd+backtick. `TriggerModifier` records which modifier opened the current session on the idle→active press, and every downstream path follows it: `registerActiveHotkeys()` registers the panel-only keys under `activeModifier.carbonKey`, and both the event tap and the watchdog test `activeModifier.eventFlag` instead of `.maskCommand`. So holding Option cycles and releasing Option commits, exactly as Command does. Two deliberate exceptions: ⌥⌘H (hide others) stays `cmdKey | optionKey` under either session, because the session holds one of the pair and the user adds the other. Backtick is skipped in the swallow loop during an Option session, since Option+backtick is already registered globally and Carbon will not hand one key+modifier pair to two registrations. Option+backtick raises the backmost non-minimized `AXWindow` rather than synthesizing a keystroke, because the physically-held Option leaks into a synthetic event and stops the system shortcut matching
...
- Keys: `grayscaleIcons`, `showMenuBarIcon` (defaults to true), `showDeclutterTip` (defaults to true), `hideMinimizedOnlyApps` (defaults to true), `limitRecentApps`, `recentAppsLimit` (defaults to 7)
...
- This fork carries no donation surface. Upstream's `launchCount` / `hasDonated` keys and
  `openDonatePage()` are removed, so any values left in `com.simpleswitcher.app` are inert.
...
- Menu: Preferences… / Grayscale Icons / Hide Other Apps / Quit Switcher. The `onOpenPreferences` and `onHideOtherApps` closures call back into AppDelegate
